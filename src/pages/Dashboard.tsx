...
// ── Shared WhatsApp URL builder (mirrors Members.tsx logic) ─────────────────
const buildWhatsAppUrl = (member: any): string | null => {
  ...
  const daysLeft = member.membershipEnd
    ? Math.ceil((new Date(member.membershipEnd).getTime() - today.getTime()) / (1000 * 60 * 60 * 24))
    : null;
  ...
  if (daysLeft !== null && daysLeft >= 0 && daysLeft <= 7) {
    ...
    if (daysSinceExpiry <= 15) {
      ...
      const deadline = new Date(member.membershipEnd);
      deadline.setMonth(deadline.getMonth() + 2);
      const deadlineDate = deadline.toLocaleDateString('en-PK', { day: '2-digit', month: 'short', year: 'numeric' });
                      ...
                      <tr key={member.id} className="hover:bg-[#1a1d24]/50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="flex items-center justify-end gap-2">
                            {/* WhatsApp Button */}
                            {buildWhatsAppUrl(member) && (
                              <a
                                href={buildWhatsAppUrl(member)!}
                                target="_blank"
                                rel="noopener noreferrer"
                                title="Send WhatsApp message"
                                className="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-green-500/10 text-green-400 hover:bg-green-500/20 hover:text-green-300 transition-all border border-green-500/20"
                              >
