...
const buildWhatsAppUrl = (member: any) => {
  ...
  const expiryDate = member.membershipEnd
    ? new Date(member.membershipEnd).toLocaleDateString('en-PK', { day: '2-digit', month: 'short', year: 'numeric' })
    : '';
  ...
  if (member.status === 'LEAD') {
    ...
    if (daysSinceExpiry <= 15) {
      ...
      const deadlineDate = twoMonthDeadline.toLocaleDateString('en-PK', { day: '2-digit', month: 'short', year: 'numeric' });
...
export default function Members() {
  ...
  const newMemberNeedsPayment = !editingId && formData.planId && formData.status === 'ACTIVE';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      if (newMember && dataToSave.planId && dataToSave.status === 'ACTIVE') {
        ...
        const admissionFee = parseFloat(localStorage.getItem('admission_fee') || '0');
...
\n${uiMessage}`); } catch {}
  ...
  const submitRenew = async (e: React.FormEvent) => {
    ...
    await (window as any).api.members.update(selectedRenewMember.id, {
      ...
      membershipStart: selectedRenewMember.membershipEnd && new Date(selectedRenewMember.membershipEnd) > new Date()
    ...
    const admissionFee = isSuspended ? parseFloat(localStorage.getItem('admission_fee') || '0') : 0;
    ...
    await (window as any).api.payments.create({
      ...
      notes: isSuspended && admissionFee > 0
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {filterConfig.map(f => (
          ...
          >
            {f.label}
            <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${statusFilter === f.key ? 'bg-white/20' : 'bg-[#1a1d24]'
              }`}>
            ...
            <tbody className="divide-y divide-[#2a2e37]">
              {loading ? (
                ...
                filteredMembers.map((member) => {
                  const daysLeft = getDaysUntilExpiry(member.membershipEnd);
                  const isExpiringSoon = daysLeft !== null && daysLeft >= 0 && daysLeft <= 7 && member.status === 'ACTIVE';
                    ...
                    <tr key={member.id} className="hover:bg-[#1a1d24]/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1">
                          <span className={`px-2.5 py-1 rounded-full text-xs font-medium w-fit ${member.status === 'ACTIVE' ? 'bg-green-500/10 text-green-400 border border-green-500/20' :
                          ...
                          {isExpiringSoon && (
                            <span className="text-xs text-amber-400 font-medium">⚠ Expires in {daysLeft}d</span>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button onClick={() => openRenewModal(member)} className="text-primary-400 hover:text-primary-300 transition-colors font-medium text-sm">Renew</button>
                          <button onClick={() => openModal(member)} className="text-gray-400 hover:text-white transition-colors text-sm">Edit</button>
                          <button onClick={() => handleDelete(member.id)} className="text-gray-400 hover:text-red-400 transition-colors text-sm">Delete</button>
                        </div>
                      </td>
                    </tr>
                  );
                ...
                return (
                  <div className="border-t border-[#2a2e37] pt-4 space-y-3">
                    ...
                    <div>
                      ...
                      <select className="input-field" value={createPaymentMethod} onChange={e => setCreatePaymentMethod(e.target.value)}>
                    ...
                    <div className="bg-[#0f1115] p-3 rounded-lg border border-[#2a2e37] text-sm space-y-1">
                      <div className="flex justify-between text-gray-400">
                        <span>Plan Fee:</span>
                        <span className="text-white">Rs {selectedPlan?.price?.toFixed(2) || '0.00'}</span>
                      </div>
                      {admissionFee > 0 && (
                        <div className="flex justify-between text-gray-400">
                          <span>Admission Fee:</span>
                          <span className="text-white">Rs {admissionFee.toFixed(2)}</span>
                        </div>
                      )}
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="glass w-full max-w-sm rounded-2xl p-6 border border-[#2a2e37] shadow-2xl relative animate-in zoom-in-95 duration-200">
              ...
              <p className="text-sm text-gray-400 mb-4">
                ...
                {isSuspended && (
                  <span className="block mt-1 text-orange-400 text-xs">⚠ Suspended member — admission fee will be charged.</span>
                )}
              </p>
              ...
              <form onSubmit={submitRenew} className="space-y-4">
                ...
                {selectedPlan && (
                  <div className="bg-[#0f1115] p-3 rounded-lg border border-[#2a2e37] text-sm space-y-1">
                    <div className="flex justify-between text-gray-400">
                      <span>Plan Fee:</span>
                      <span className="text-white">Rs {selectedPlan.price.toFixed(2)}</span>
                    </div>
                    ...
                    <div className="flex justify-between text-primary-400 font-bold border-t border-[#2a2e37] pt-1 mt-1">
                      ...
                      <span className="text-white text-lg">Rs {totalAmount.toFixed(2)}</span>
                    </div>
                  </div>
                )}
