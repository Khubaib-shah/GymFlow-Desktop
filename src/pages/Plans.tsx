...
export default function Plans() {
  ...
  const handleDelete = async (id: string) => {
    if (await showConfirm('Are you sure you want to delete this plan?')) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Memberships</h1>
          <p className="text-gray-400 mt-1">Configure pricing and durations for your gym memberships.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {loading ? (
          <div className="col-span-full text-center py-10 text-gray-500">Loading memberships...</div>
        ) : plans.length === 0 ? (
          <div className="col-span-full text-center py-10 text-gray-500 glass rounded-xl">No memberships configured yet.</div>
        ) : (
          plans.map(plan => (
            <div key={plan.id} className="glass rounded-xl p-6 border border-[#2a2e37] relative group overflow-hidden">
              ...
              <div className="mt-6 flex gap-3">
                <button onClick={() => openModal(plan)} className="flex-1 btn-secondary text-sm">Edit Membership</button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="glass w-full max-w-sm rounded-2xl p-6 border border-[#2a2e37] shadow-2xl relative">
            <h2 className="text-xl font-bold text-white mb-4">{editingId ? 'Edit Membership' : 'Add Membership'}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-400 mb-1">Membership Name</label>
